...
from keras.layers import MaxPooling3D, Flatten, Dropout
...
DATA_PATH_REMOTE = '/home/javierrp/ivan/dataset/equalized_by_ad.h5'
DATA_PATH_LOCAL = '/Users/XT/Documents/PhD/Granada/neuroimaging/Python_scripts/CNN/dataset/equalized_by_ad.h5'


def get_train_data(filepath=DATA_PATH_REMOTE):
...
def model(_x, _y):
    ...
    x = Conv3D(filters=128, kernel_size=3, activation='selu')(x)
    x = Conv3D(filters=192, kernel_size=3, activation='relu')(x)
    x = MaxPooling3D()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.7)(x)
...
if __name__ == '__main__':
    ...
    model.fit(x=features, y=labels, batch_size=48, epochs=500, validation_split=0.1)
